...
class EleutherEvalRecipe(EvalRecipeInterface):
    ...
    def setup(self, cfg: DictConfig) -> None:
        ...
        import os
        subnet_config_path = cfg.ir.subnet_config_path
        subnet_config = torch.load(subnet_config_path)
        # subnet_config = ir.sample_min_elastic_config(
        def replace_dim_keys(config_dict):
            updated_config = {}
            for key, value in config_dict.items():
                # new_value = value.copy()
                new_value = {}
                new_value['in_features'] = value['in_dim']
                new_value['out_features'] = value['out_dim']
                new_value['bias'] = value['use_bias']
                # if 'in_dim' in new_value:
                #     new_value['in_features'] = new_value.pop('in_dim')
                # if 'out_dim' in new_value:
                #     new_value['out_features'] = new_value.pop('out_dim')
                updated_config[key] = new_value
            return updated_config
        subnet_config = replace_dim_keys(subnet_config)
        
        
        for layer_idx, layer_config in subnet_config.items():
            for k, v in layer_config.items():
                if "in_dim" in k:
                    layer_config[k.replace("in_dim", "in_features")] = v
                    del layer_config[k]
                
        with training.set_default_dtype(self.dtype), self.device:
            model = ir.create_subnet(subnet_config)
